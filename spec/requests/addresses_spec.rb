...
RSpec.describe 'Addresses', type: :request do
  describe 'GET /search' do
    it 'returns success with good address' do
      allow_any_instance_of(AddressesHelper).to receive(:fetch_address).and_return(
        [{ address: '124 Main St', longitude: 23, latitude: 20, postcode: 77_631, country_code: 'us' }]
      )
    ...
    it 'handles an empty search set result' do
      allow_any_instance_of(AddressesHelper).to receive(:fetch_address).and_return([])
